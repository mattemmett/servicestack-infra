...
3. `servicestack` uploads `.env.prod`, `docker-compose.yml`, and app `nginx.conf` to `/opt/servicestack/app` on the infra-managed host
4. `servicestack` calls `servicestack-infra/scripts/deploy-via-ssm.sh` to perform the generic host-side compose rollout
5. `servicestack` runs post-deploy smoke checks through host-routed endpoints
...
- the uploaded `.env.prod` payload currently includes `GHCR_IMAGE` and `ENV_FILE=.env.prod` so compose interpolation resolves the correct runtime env file on host
